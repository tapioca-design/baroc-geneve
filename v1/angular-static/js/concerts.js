myApp.controller('ConcertsListController', ['$rootScope','$scope','$http', "$routeParams",'$location','Data','Search','Const', function($rootScope,$scope,$http,$routeParams,$location, Data, Search, Const) { 
      ...
      //var retrievedObject = localStorage.getItem('worksOrderedByFirstPerformance');
...
//get work
  if (localStorage.getItem("work/"+workId) === null) {
      console.log("work not here");
      $http.get(Const.baseUrl+'/symfony/web/api/work/'+workId).
        success(function(work) {
            Data.headerTitle=work.name;
            localStorage.setItem("work/"+workId, JSON.stringify(work));
            $scope.getLandscapeBkgPath(work,"works",work.name_url);
        });
  } else {
        console.log("work local:");

        var workLS = localStorage.getItem("work/"+workId);
        workLS = JSON.parse(workLS);
        Data.headerTitle=workLS.name;
        $scope.getLandscapeBkgPath(workLS,"works",workLS.name_url);
  }
...
//get performances
  if (localStorage.getItem('work/'+workId+'/performances') === null) {
      console.log("performances not here");
      $http.get(Const.baseUrl+'/symfony/web/api/work/'+workId+'/performances').
        success(function(performances) {
            localStorage.setItem('work/'+workId+'/performances', JSON.stringify(performances));
            $scope.Data = Data;
            $scope.performances = performances;
        });
  } else {
        console.log("performances local:");
        var performancesLS = localStorage.getItem('work/'+workId+'/performances');
        $scope.performances = JSON.parse(performancesLS);
  }
